...
def get_period_list(
	...
	if periodicity in ["Yearly", "Half-Yearly", "Quarterly", "Monthly"]:
		...
		for dimension in dimesion_list:
			period_dct = frappe._dict({
					"key": dimension,
					"label": dimension,
			...
			if not ignore_fiscal_year:
				period_dct.to_date_fiscal_year = get_fiscal_year(year_end_date, company=company)[0]
				period_dct.from_date_fiscal_year_start_date = get_fiscal_year(year_start_date, company=company)[1]

			period_list.append(period_dct)
	...
	for entries in itervalues(gl_entries_by_account):
		for entry in entries:
			...
			for period in period_list:
				...
				if dimesion_list:
					cond = entry.get(scrub(filters.get('periodicity'))) == period.key
				else:
					cond = (
				...
				if entry.posting_date <= period.to_date:
					if (accumulated_values or entry.posting_date >= period.from_date) and cond:
...
def set_gl_entries_by_account(
	...
	if accounts:
		...
		if dimesion_list:
			additional_select_fields = ", {}".format(scrub(filters.get("periodicity")))

			additional_conditions += " and {} in ({})".format(
				scrub(filters.get("periodicity")),
				", ".join(frappe.db.escape(d) for d in dimesion_list)
			)
		...
		gl_entries = frappe.db.sql(
			"""select posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency{additional_select_fields}
			from `tabGL Entry`
			where company=%(company)s
			{additional_conditions}
			and posting_date <= %(to_date)s
