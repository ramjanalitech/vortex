...
from vortex.vortex.report.financial_statements import (
	get_columns,
	get_data,
...
def execute(filters=None):
	dimesion_list = get_accounting_dimesion_data(filters)
...
def get_accounting_dimesion_data(filters):
	dimension_filters, dimension_list = {}, []
	
	if (filters.periodicity not in ["Yearly", "Half-Yearly", "Quarterly", "Monthly"] and 
			frappe.db.exists("Accounting Dimension", {"document_type": filters.periodicity})):
				periodicity = scrub(filters.periodicity)
				if filters.get(periodicity):
					dimension_filters_data = filters.get(periodicity)
					if frappe.get_cached_value("DocType", filters.periodicity, "is_tree"):
						dimension_filters_data = get_dimension_with_children(
							filters.periodicity, filters.get(periodicity)
						)
					dimension_filters = {"name": ("in", dimension_filters_data)}

				dimension_list = frappe.db.get_list(filters.periodicity, dimension_filters, pluck= "name", order_by= "name")
	
	return dimension_list

@frappe.whitelist()
def _get_accounting_dimensions():
	return frappe.db.get_list("Accounting Dimension", {"disabled": 0}, pluck= "name", order_by= "name")